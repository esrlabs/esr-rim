  ...
  class SyncModuleHelper < ModuleHelper
    ...
    def parse_ignored_files(session, out, e)
      ...
      out.gsub(/warning:.*will be replaced.*\r?\n.*\r?\n/, '').split(/\r?\n/).each do |l|
        if first_line && l.include?("LF will be replaced by CRLF the next time Git touches it")
          next
        elsif first_line && !l.include?(".gitignore")
          raise text || RimException.new("Cannot parse ignored files after git add:\n#{out}")
        end
